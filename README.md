...
```bash
# 录音时用的不是 USB 时要指定模式（决定分析通带）
python main.py analyze data/recordings/xxx.wav --mode AM
```
...
| 指标 | 含义 |
|------|------|
| **带内 SNR** | 只在解调通带内估算（噪声基底取带内功率的低分位数）。清晰语音一般在 20 dB 以上，纯底噪是负值 |
| **占用带宽** | 扣除底噪后包含 90% 能量的频率跨度。纯音只有几十 Hz，语音在 1-2.5 kHz |
| **包络音节率** | 语音的音节率集中在 2-8 Hz，FSK/PSK 是恒包络（深度极低），莫尔斯键控深度接近 100% |
| **置信度** | 各类别得分的绝对值与领先幅度。低于 0.35 时输出 `UNKNOWN`——特征不足以判定就不硬猜 |

可能的调制类型：`USB_VOICE` / `LSB_VOICE` / `AM_VOICE` / `CW` / `CARRIER`（未调载波）/
`FSK` / `PSK` / `NOISE` / `UNKNOWN`。USB 还是 AM 由接收机的解调模式决定，不从音频里猜。
...
| 区域 | 功能 |
|------|------|
| **左侧面板** | 频率快速选择（按网络分组，标出最近有信号的频率）、监听参数、静噪在线调整、节点管理 |
| **中央区域** | 仪表盘（频率、S-meter dBm/S 级、带内 SNR、RMS、信号数）+ 实时频谱与瀑布图 + RMS 曲线 |
| **右侧面板** | 三个标签页：**实时**信号日志 / **录音**浏览回放 / **统计** |
...
5. 中央区域实时显示频谱、瀑布图和 RMS 曲线；仪表盘给出 S-meter 与带内 SNR
6. 检测到信号时，右侧"实时"标签会自动追加日志，可直接点 ▶ 回放、点 ▤ 看频谱图
7. 点击 **Stop Monitoring** 停止

### 实时频谱与瀑布图

中央区域上半是频谱曲线，下半是向下滚动的瀑布图（最新的在上），蓝色阴影标出当前解调模式的
通带（USB 是 300-3000 Hz）。右上角实时显示峰值频率、带内 SNR 和噪声基底。

> 瀑布图是信息密度最高的控件：稳定的水平亮线是未调制载波，规律的短横是 CW 键控，
> 占满通带的连续色块是语音或数据。

### 录音浏览与回放

"录音"标签页列出历史信号，每条包含时长、带内 SNR、调制类型与置信度、占用带宽、S-meter：

- **▶** 直接在浏览器里播放这段录音
- **▤** 展开这段录音的频谱图缩略图
- 顶部可按时间范围、最低 SNR 筛选，或只看有录音文件的记录

### 静噪在线调整

左侧 **Squelch** 区可以直接拖滑块改开启/关闭阈值，**立刻作用到正在运行的监听**，不用停下重来。
面板同时显示服务器实测的底噪 RMS（静噪关闭期间的低分位数）和建议阈值：

- **按底噪设定** — 一键把阈值设成 `底噪 × 2`（约 +6 dB），换节点、换时段后重新标定只要点一下
- RMS 曲线上会同时画出噪声基底线、静噪开启线和关闭线，一眼看出阈值卡得对不对

> 阈值和节点的 AGC 设置有关，换个节点原来的数就不对了。与其猜，不如让它按实测底噪来定。
...
```yaml
squelch:
  open_threshold: 0.10    # 降低 = 更灵敏（检测更多弱信号，但可能误触发）
  close_threshold: 0.085  # 关闭阈值（低于开启阈值，形成滞后）
  tail_time: 3.0          # 信号消失后继续录 3 秒
```

> 不确定该填多少时，先启动 Web 界面跑几分钟，左侧 Squelch 面板会显示实测底噪和建议值
> （底噪 × 2 ≈ +6 dB），点"按底噪设定"即可，改完立刻生效。
...
A: 那条红色虚线是**静噪阈值线**，对应 `config.yaml` 中的 `open_threshold`（默认 0.10）。
- RMS 波形**低于**该线 → 只是背景底噪，不录音
- RMS 波形**高于**该线 → 检测到有效信号，自动开始录音

图上还有两条线：更淡的红色虚线是关闭阈值（`close_threshold`，形成滞后防止频繁开关），
灰色虚线是服务器实测的**噪声基底**。三条线放在一起就能直接判断阈值卡得对不对——
阈值应该在噪声基底上方约 6 dB（≈ 2 倍）。

> **注意：** 底噪 RMS 取决于对面节点的 AGC 设置，换个节点这个数就变了。不用手改配置文件再重启：
> 左侧 Squelch 面板可以在线调整，或者直接点"按底噪设定"让它按实测底噪标定。

### Q: S-meter 一直显示 -160 dBm / 录音里有低频嗡声？
A: 这是 v1.3.0 之前的 bug，已修复。SND 帧的 body 布局被写错了（seq 是 4 字节小端而不是
2 字节大端），导致整个帧偏移 2 字节：S-meter 位置读到的是帧计数器的高位字节，
音频每帧还多出 1 个假样本，在录音里表现为 23.4 Hz（= 12000/512 帧率）及其谐波的嗡声。
升级后 S-meter 是真实读数，嗡声消失。

> 注意：**修复只对新录音生效**。此前录制的 WAV 文件里已经写入了那个假样本，
> 数据库里的旧 S-meter 值也仍然是帧计数器的残留，不要拿它们和新数据混在一起比较。
